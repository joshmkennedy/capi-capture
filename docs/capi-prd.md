...
1. The user records screen captures.
2. Capi registers each completed recording as a source.
3. The browser editor shows available sources.
4. The user trims and arranges clips on a timeline.
5. The user previews the presentation.
6. The user exports a final shareable file.
...
6. As a maker, I want captured sources to remain unchanged, so that I can edit without damaging the original media.
7. As a maker, I want to see all sources for the current session, so that I can choose what belongs in the presentation.
8. As a maker, I want sources to show readable names, so that I can identify them quickly.
9. As a maker, I want sources to expose duration, so that I understand how much material I captured.
10. As a maker, I want to place part of a source on the timeline, so that I can include only the useful section.
11. As a maker, I want a clip to reference a source range, so that one source can be trimmed without changing the source itself.
12. As a maker, I want to trim the start of a clip, so that I can remove setup time.
13. As a maker, I want to trim the end of a clip, so that I can remove trailing noise.
14. As a maker, I want trims to respect minimum clip length, so that I do not accidentally create unusable clips.
15. As a maker, I want clips to sequence cleanly on the timeline, so that my presentation plays without confusing gaps.
16. As a maker, I want to reorder clips, so that I can tell the story in the right order.
17. As a maker, I want to nudge clip boundaries, so that I can adjust timing with small corrections.
18. As a maker, I want to preview the current timeline, so that I can verify the presentation before exporting.
19. As a maker, I want playback to move from clip to clip, so that preview reflects the exported result.
20. As a maker, I want the preview to show the correct source time for each clip, so that trims are easy to trust.
21. As a maker, I want to seek through the timeline, so that I can inspect specific moments.
22. As a maker, I want to jump to the start or end of the presentation, so that review is efficient.
23. As a maker, I want to see active clip details, so that I can understand what I am editing.
24. As a maker, I want to see source in and source out values, so that trim decisions are visible.
25. As a maker, I want to see timeline placement, so that I understand where a clip starts.
26. As a maker, I want to export the presentation, so that I can share it with a client, teammate, or stakeholder.
27. As a maker, I want export to stitch clips in timeline order, so that the final file matches my edit.
28. As a maker, I want export to re-encode for compatibility, so that the result plays reliably for recipients.
29. As a maker, I want export errors to be readable, so that I know what failed.
30. As a maker, I want a successful export to tell me where the file was saved, so that I can share it immediately.
31. As a maker, I want the current session to feel temporary, so that I can make quick presentations without managing projects.
32. As a maker, I want Capi to avoid professional editing jargon, so that the tool feels simple to learn.
33. As a maker, I want the interface to use terms like sources, clips, timeline, and presentation consistently, so that I build a correct mental model.
34. As a maker, I want mocked sources during prototype development, so that editor behavior can be improved before full capture integration exists.
35. As a developer, I want mocked sources behind the `CapiClient` boundary, so that real session sources can replace them later without changing editor components.
36. As a developer, I want sources and clips modeled separately, so that timeline edits do not corrupt source identity.
37. As a developer, I want the editor and runtime to share export contracts, so that payload drift does not break export.
38. As a developer, I want timeline behavior in a deep, testable module, so that trimming and sequencing can be changed safely.
39. As a developer, I want export planning separate from ffmpeg execution, so that validation can be tested without invoking external binaries.
40. As a developer, I want ffmpeg execution isolated behind a small boundary, so that external-process failure handling stays contained.
41. As a developer, I want the Vite config to mount runtime middleware without owning runtime implementation, so that the prototype can keep using Vite while preserving architecture.
42. As a developer, I want the local runtime to own capture and export, so that browser code does not learn about macOS processes or filesystem details.
43. As a developer, I want the browser editor to own only editor interactions, so that UI changes do not affect capture and export internals.
44. As a developer, I want shared terminology documented, so that future agents and contributors name concepts consistently.
45. As a developer, I want a clear path from mocked sources to captured sources, so that the prototype can become useful without a rewrite.
46. As a developer, I want session lifecycle to be explicit, so that child processes and temporary files can be cleaned up correctly.
47. As a developer, I want source registration to be distinct from file watching, so that a completed `.mov` becomes available only after it is safe to edit.
48. As a developer, I want export output to be deterministic enough to debug, so that support issues can be traced from payload to file.
49. As a developer, I want runtime modules to expose small interfaces, so that tests can focus on behavior instead of internal helpers.
50. As a developer, I want architecture documentation to match the code, so that future implementation work does not restart the same decisions.
